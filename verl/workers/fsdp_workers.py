...
class CriticWorker(Worker):
    def __init__(self, config):
        ...
        self.config.ppo_mini_batch_size //= dist.get_world_size() // self.ulysses_sequence_parallel_size
        if self.config.ppo_micro_batch_size is not None:
            self.config.ppo_micro_batch_size //= dist.get_world_size() // self.ulysses_sequence_parallel_size
            self.config.forward_micro_batch_size //= dist.get_world_size() // self.ulysses_sequence_parallel_size
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        self.tokenizer = get_tokenizer(
            config.model.tokenizer_path, trust_remote_code=config.model.get("trust_remote_code", False)
        )
        ...
        trust_remote_code = False
        critic_model_config = AutoConfig.from_pretrained(config.model.path, trust_remote_code=trust_remote_code)
...
class RewardModelWorker(Worker):
    ...
    def __init__(self, config):
        ...
        if self.config.micro_batch_size is not None:
            self.config.micro_batch_size //= dist.get_world_size()
    ...
    def _build_model(self, config):
        if self.config.model.input_tokenizer is None:
            ...
            self.input_tokenizer = get_tokenizer(
                config.model.input_tokenizer, trust_remote_code=config.model.get("trust_remote_code", False)
            )
            self.tokenizer = get_tokenizer(
                config.model.path, trust_remote_code=config.model.get("trust_remote_code", False)
            )

        trust_remote_code = config.model.get("trust_remote_code", False)
        model_config = AutoConfig.from_pretrained(config.model.path, trust_remote_code=trust_remote_code)
